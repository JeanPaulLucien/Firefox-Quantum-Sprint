...
/* Try to comlete in compatible modee */
function makeCompatible(ffVer, mfns) {
	// mfns - id to detect a string 
	if (ffVer < 44) {
		console.log('Старики идут в бой');
		switch (mfns) {
			case 1: var map; break;
			case 2: 
				var API_KEY_MAPS = 'AIzaSyCtHdTVnmaA1ZQ-1wdaKoVEBtlTT020yXQ',
				    API_URL = 'https://maps.googleapis.com/maps/api/js?key=' + API_KEY_MAPS + '&callback=mapCallback';
					break;
			case 3:
				var hasContainerEventList = !!document.querySelector('.map-sites'),
				    hasContainerEventDetail = !!document.querySelector('.event-details');
				break;
		}
	}
	else {
		console.log('Молодежь идет в бой');
		switch (mfns) {
			case 1: //let map; 
				break;
			case 2: 
